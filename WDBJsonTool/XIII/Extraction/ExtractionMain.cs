...
namespace WDBJsonTool.XIII.Extraction
{
    internal abstract class ExtractionMain
    {
        public static WDBFile StartExtraction(string inWDBfile, bool shouldIgnoreKnown)
        {
            ...
            {
                WDBName = Path.GetFileNameWithoutExtension(inWDBfile)
            };
            ...
            using (var wdbReader = new BinaryReader(File.Open(inWDBfile, FileMode.Open, FileAccess.Read)))
            {
                ...
                wdbFile.Records = wdbVars.IsKnown ? RecordsParser.ParseRecordsWithFields(wdbReader, wdbVars) : RecordsParser.ParseRecordsWithoutFields(wdbReader, wdbVars);
            }
